...
highlight CKeywParNext ctermbg=green guibg=green
...
au BufWinEnter *.c,*.h let w:m4=matchadd('CKeywSpaceNext', '\(if\|while\|return\|const\|enum\|struct\|typedef\|extern\|unsigned\)[^ ]', -1)
au BufWinEnter *.c let w:m5=matchadd('CKeywParNext', '\(if\|while\|return\) [^(]', -1)
set mouse=a
